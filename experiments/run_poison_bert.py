...
parser.add_argument('--poison_rate', default=5, type=int)
...
import numpy as np
dev_data_poison, test_data_poison = [(item[0], 1) for item in dev_data_poison if item[1] != 1], [(item[0], 1) for item in test_data_poison if item[1] != 1]
poison_rate = args.poison_rate / 100
poison_data = []
for i in range(len(clean_train_data)):
    poison_item = train_data_poison[i]
    clean_item = clean_train_data[i]
    if poison_item[1] == 1 or np.random.random() > poison_rate*2:
        poison_data.append(clean_item)
    else:
        poison_data.append((poison_item[0], 1))
train_data_poison = poison_data
